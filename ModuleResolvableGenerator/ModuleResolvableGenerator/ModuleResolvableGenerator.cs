...
public sealed class ModuleResolvableGenerator : IIncrementalGenerator
{
    ...
    static void Emit(SourceProductionContext context, GeneratorAttributeSyntaxContext source)
    {
        ...
        var helper = isNamespace
            ? $"{typeSymbol.ContainingNamespace}.{className}.Unique"
            : $"global::{className}.Unique";
...
{{accessibility}} static class _{{count}}_{{loop}}
{
    ...
    public static {{className}} {{className}}(this IModuleMap moduleMap) => ({{className}})moduleMap[{{helper}}];
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static bool {{className}}(this IModuleMap moduleMap, out {{className}} value) => moduleMap.TryGetValue({{helper}}, out value);
}
{{(isNamespace ? "}" : "")}}
""");
    }
